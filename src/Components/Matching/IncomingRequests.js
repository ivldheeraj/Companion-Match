...
const IncomingMatchRequests = ({ studentId, statusFilter }) => {
  ...
  useEffect(() => {
    if (eventId) {
      fetchEventData(eventId); // Fetch event data when eventId is available
    }
    fetchRequests(); // Fetch match requests
  }, [studentId, eventId]);
                ...
                <Card.Body className="text-center">
                  ...
                  {eventData && ( // Display event name if event data is available
                    <Card.Subtitle className="mb-2 text-muted">
                      Event: {eventData.EventTitle} {/* Displaying the event title */}
                    </Card.Subtitle>
                  )}
